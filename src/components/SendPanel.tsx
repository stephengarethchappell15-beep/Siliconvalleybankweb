...
export const SendPanel: React.FC<SendPanelProps> = ({ user, onSuccess }) => {
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    // Requirement 6: Check if outgoing security code is active
    ...
    if (user.role !== 'admin' && !fourDigitCode.trim()) {
      setError('Please enter your 4-Digit Outgoing Security Code.');
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Transfer request failed. Please check the transaction details.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl relative">
        <form onSubmit={handleTransfer} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* 6. 4-Digit Outgoing Security Code */}
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-2">
                4-Digit Outgoing Security Code <span className="text-rose-400">*</span>
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-slate-500 font-mono tracking-widest outline-none transition-colors"
      ...
      {/* Requirement 6 Modal: Deposit Prompt when Outgoing Security Code is not active */}
